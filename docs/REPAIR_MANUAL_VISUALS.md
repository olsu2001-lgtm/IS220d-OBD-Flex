...
## Third image batch: reviewed source figures (2026-09-15)

Seven additional RM0150 figures have now been located and visually reviewed for
existing Vikadiag diagnostic items. Their exact source image path, source HTML,
component identity and SHA-256 hashes are recorded in
`src/is220d-repair-manual-visual-candidates.js`.

This batch is deliberately marked **source-identified**, not `available`: the
corresponding PNG bytes have not yet been committed under
`assets/repair-manual/`, so the normal visual renderer must not pretend these
pictures are already packaged in Flex.

| Drive row | Diagnostic component | RM0150 image | Manual section |
| ---: | --- | --- | --- |
| 11 | Laturi / generator | `a132252e01.png` | `rm000001arc007x.html` / COMPONENTS |
| 12 | Starttimoottori | `a132250e01.png` | `rm00000167v005x.html` / COMPONENTS |
| 21 | Välijäähdytin | `a132945e01.png` | `rm0000019sa007x.html` / COMPONENTS |
| 25 | Turboahdin ja liitännät | `a122209e03.png` | `rm0000019s7004x.html` / COMPONENTS |
| 29 | Alipaineen säätöventtiili | `a122208e01.png` | `rm000001hnk003x.html` / COMPONENTS |
| 61 | Vesipumppu | `a125620e02.png` | `rm000001bjy006x.html` / COMPONENTS |
| 66 | Jäähdytin ja puhallinkokonaisuus | `a121167e01.png` | `rm000001bjv006x.html` / COMPONENTS |

The generator figure uses the repair manual's `GENERATOR` terminology while the
catalog component remains `engine.alternator`. The turbo, intercooler and vacuum
regulator figures are mapped only to their reviewed component IDs. The water
pump and radiator figures were selected from the matching diesel/2AD-looking
RM0150 component sections rather than from unrelated engine variants.
...
The next packaging step is mechanical: copy the seven exact source PNGs to their
recorded `targetAssetPath`, verify the bytes against `sourceImageSha256`, then
move only those verified mappings into `src/is220d-repair-manual-visuals.js`.
Until that happens the existing renderer continues to expose only already
packaged images.
