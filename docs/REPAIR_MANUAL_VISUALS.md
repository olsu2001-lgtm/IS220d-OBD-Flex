...
The staging registry now contains **26 diagnostic components** backed by exact **2AD-FHV** RM0150 source figures or by an exact 2AD-FHV repair step that explicitly names the mapped component. Every candidate records the source image, source HTML page, image and HTML SHA-256, diagnostic component identity, Drive row and intended local asset path in `src/is220d-repair-manual-visual-candidates.js`.
...
| 22 | Imusarja | `a131695.png` | `rm0000019y9009x.html` / ENGINE ASSEMBLY / REMOVE INTAKE MANIFOLD |
| 23 | Imusarjan tiiviste | `a131695.png` | `rm0000019y9009x.html` / ENGINE ASSEMBLY / REMOVE INTAKE MANIFOLD |
...
| 27 | Turbon/pakopuolen tiivistekohdat | `a122201e01.png` | `rm0000019s7004x.html` / TURBOCHARGER / COMPONENTS |
...
| 36 | Matalapainepuolen polttoaineletkut | `a134252.png` | `rm0000024nj000x.html` / FUEL FILTER / REPLACEMENT |
...
The latest continuation added rows **22, 23, 27 and 36** after byte-for-byte extraction from the ISO. `a131695.png` is tied to the 2AD-FHV `REMOVE INTAKE MANIFOLD` step, whose text explicitly removes both the intake manifold and gasket. `a134252.png` is tied to the 2AD-FHV fuel-filter replacement procedure and shows three fuel hoses at the No. 2 fuel-tube clamp. These figures are orientation/inspection aids; neither one by itself proves leakage, restriction or component failure.

Row 28 deliberately describes the VRV-area hoses as a local example rather than claiming that one drawing is the complete vacuum-routing diagram. Row 64 has a dedicated 2AD-FHV removal figure explicitly identifying the `V-RIBBED BELT TENSIONER ASSEMBLY`.

### Items intentionally still pending

No image candidate is added merely because a nearby manual page looks plausible. For example, the 2AD-FHV on-vehicle inspection gives a textual air-filter inspection procedure but does not provide an air-filter figure in that step, so row 19 remains pending. The exact role/location of row 31 `FILTER, GAS` is still insufficiently identified for a component-specific image mapping, so it also remains pending.
...
Candidate tests explicitly reject those 4GR-FSE source pages. Shared drawings are reused only where the labelled content or exact repair step supports the mapped component identity. A picture does not change diagnostic readiness, authorize a signal, create a threshold or prove a component faulty.
