...
function evaluatePoint(point, signalStates, progress) {
  ...
  if (point.requiresPhysicalConfirmation && !manual) {
    ...
    status = "evidence-collected";
    reason = `Kaikki pisteen nykyiset sallitut signaalit havaittiin (${observed.length}/${authorized.length}).`;
  } else if (observed.length > 0 || attempted.length > 0) {
    status = "partial";
    reason = `Sallittua evidenssiä saatiin ${observed.length}/${authorized.length}; ${attempted.length} signaalia yritettiin.`;
  }
...
export function buildIs220dInspectionExecutionState(coverage, progress = emptyProgress(), groupId = "all") {
  ...
  const points = IS220D_ALL_INSPECTION_POINTS
    ...
    .map(point => evaluatePoint(point, signalStates, progress));
...
export function buildIs220dInspectionExecutionSummaryHtml(state) {
  ...
  return `<div class="bom-exec-summary-head"><div><strong>Tarkastuspisteiden suoritus</strong><span>${Number(state?.pointCount || 0)} pistettä · ${Number(state?.componentCount || 0)} komponenttia</span></div><small>Tilat muodostuvat kerätystä read-only-datasta ja fyysisten pisteiden omista kuittauksista.</small></div><div class="bom-exec-summary-grid"><span class="done">EVIDENSSI ${Number(summary.evidenceCollected || 0)}</span><span class="partial">OSITTAIN ${Number(summary.partial || 0)}</span><span class="physical">FYYSINEN ${Number(summary.requiresPhysicalConfirmation || 0)}</span><span class="waiting">ODOTTAA SIGNAALIA ${Number(summary.awaitingVerifiedSignal || 0)}</span><span class="idle">EI ALOITETTU ${Number(summary.notStarted || 0)}</span>${Number(summary.deviationsFound || 0) ? `<span class="deviation">POIKKEAMIA ${Number(summary.deviationsFound || 0)}</span>` : ""}</div>`;
}
...
function renderPointState(point) {
  ...
  const actions = point.requiresPhysicalConfirmation
    ...
    : "";
  box.innerHTML = `<div class="bom-exec-state-line"><strong>${escapeHtml(point.statusLabel)}</strong>${finding}</div><div class="bom-exec-reason">${escapeHtml(point.reason)}</div>${awaiting}${actions}`;
...
function ensureStyles() {
  ...
  style.textContent = `
    ...
    .bom-exec-point-state{margin:7px 0 0;padding:7px;border:1px solid var(--line-soft);border-radius:8px;background:var(--surface-inset)}.bom-exec-state-line{display:flex;align-items:center;gap:5px;flex-wrap:wrap}.bom-exec-reason,.bom-exec-awaiting{margin-top:4px;color:var(--muted);font-size:8px;line-height:1.35}.bom-exec-awaiting{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}
    ...
    .bom-exec-point-state.done strong,.bom-exec-summary-grid .done{border-color:var(--success-border);color:var(--success)}.bom-exec-point-state.partial strong,.bom-exec-summary-grid .partial{border-color:var(--warning-border);color:var(--warning)}.bom-exec-point-state.physical strong,.bom-exec-summary-grid .physical{color:var(--info)}.bom-exec-point-state.waiting strong,.bom-exec-summary-grid .waiting{color:var(--muted)}.bom-exec-summary-grid .deviation,.bom-exec-finding{border-color:var(--warning-border);color:var(--warning)}
  `;
  document.head.append(style);
}

function currentState() {
  return buildIs220dInspectionExecutionState(latestCoverage, readIs220dInspectionPointProgress(), selectedGroupId());
}

function render() {
  if (typeof document === "undefined") return buildIs220dInspectionExecutionState(latestCoverage, readIs220dInspectionPointProgress(), "all");
  ensureStyles();
