...
export function buildIs220dStartingChargingInspectionScopeHtml(scope) {
  ...
  return `<div class="bom-start-point-summary"><strong>Startti / lataus / kampisignaali</strong><span>${scope.pointCount} tarkastuspistettä · ${scope.componentCount} komponenttia</span></div>${scope.points.map(point => `<article class="bom-start-point-card" data-bom-start-point="${escapeHtml(point.id)}"><div class="bom-start-point-head"><div><span>${escapeHtml(kindLabel(point.kind))}</span><strong>${escapeHtml(point.label)}</strong></div><small>${escapeHtml(point.states.map(state => state.label).join(" · "))}</small></div><div class="bom-start-point-component">${escapeHtml(point.componentLabel)} · PNC ${escapeHtml(point.pnc)} · OE ${escapeHtml(point.oe)}</div><p>${escapeHtml(point.instruction)}</p><div class="bom-start-point-pattern"><strong>Odotettu kuvio</strong><span>${escapeHtml(point.expectedPattern)}</span></div><div class="bom-start-point-limit"><strong>Raja</strong><span>${escapeHtml(point.limitation)}</span></div><details><summary>Jatkotoimi ja lähde</summary><div>${escapeHtml(point.physicalFollowUp)}</div><div>Evidenssi nykytestissä ${point.evidenceSummary.currentWideDiagnostic}/${point.evidenceSummary.signalCount}</div><div class="bom-start-point-source">${escapeHtml(point.sourceSheet)} · rivit ${escapeHtml(point.sourceRows.join(", "))}</div></details></article>`).join("")}`;
}
