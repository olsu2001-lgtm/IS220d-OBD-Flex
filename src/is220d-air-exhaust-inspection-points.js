...
export function buildIs220dAirExhaustInspectionScopeHtml(scope) {
  ...
  return `<div class="bom-ae-point-summary"><strong>Ilma / turbo / alipaine / DPNR-lisäkohteet</strong><span>${scope.pointCount} tarkastuspistettä · ${scope.componentCount} komponenttia</span></div>${scope.points.map(point => `<article class="bom-ae-point-card" data-bom-ae-point="${escapeHtml(point.id)}"><div class="bom-ae-point-head"><div><span>${escapeHtml(kindLabel(point.kind))}</span><strong>${escapeHtml(point.label)}</strong></div><small>${escapeHtml(point.states.map(state => state.label).join(" · "))}</small></div><div class="bom-ae-point-component">${escapeHtml(point.componentLabel)} · PNC ${escapeHtml(point.pnc)} · OE ${escapeHtml(point.oe)}</div><p>${escapeHtml(point.instruction)}</p>${buildIs220dRepairManualVisualsHtml(point.componentId)}<div class="bom-ae-point-pattern"><strong>Odotettu kuvio</strong><span>${escapeHtml(point.expectedPattern)}</span></div><div class="bom-ae-point-limit"><strong>Raja</strong><span>${escapeHtml(point.limitation)}</span></div><details><summary>Jatkotoimi ja lähde</summary><div>${escapeHtml(point.physicalFollowUp)}</div><div>Evidenssi nykytestissä ${point.evidenceSummary.currentWideDiagnostic}/${point.evidenceSummary.signalCount}</div><div class="bom-ae-point-source">${escapeHtml(point.sourceSheet)} · rivit ${escapeHtml(point.sourceRows.join(", "))}</div></details></article>`).join("")}`;
}
