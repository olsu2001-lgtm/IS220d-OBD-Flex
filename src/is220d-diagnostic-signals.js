 ...
 *
 * metricIds bind production-authorized evidence to existing Flex decoder/live
 * metric definitions. A later selective runner resolves these IDs through the
 * production client rather than converting catalog text into transport calls.
 */
...
function signal(definition) {
  ...
  if (definition.productionAuthorized === true) {
    if (!Array.isArray(definition.metricIds) || definition.metricIds.length === 0) throw new Error(`Production diagnostic signal ${definition.key} has no existing Flex metric binding`);
    for (const metricId of definition.metricIds) {
      if (!/^[A-Za-z][A-Za-z0-9]*$/.test(String(metricId || ""))) throw new Error(`Invalid metric ID ${metricId} for ${definition.key}`);
    }
  }
  return definition;
}
...
export const IS220D_DIAGNOSTIC_SIGNALS = deepFreeze([
  signal({ key: "engine.rpm", label: "Moottorin kierrosluku", commands: ["010C"], metricIds: ["rpm"], expectedResponsePrefix: "410C", decoder: "sae-mode01-rpm", unit: "rpm", plausibleRange: [0, 8000], evidence: "vehicle-verified", authorization: "standard-obd", productionAuthorized: true, collectedByWideDiagnostic: true, operatingStates: ["cranking", "running"], source: "SAE Mode 01 PID 0C / existing Flex decoder" }),
  signal({ key: "engine.coolant_temperature", label: "Jäähdytysnesteen lämpötila", commands: ["0105"], metricIds: ["coolant"], expectedResponsePrefix: "4105", decoder: "sae-mode01-coolant-temperature", unit: "°C", plausibleRange: [-40, 215], evidence: "vehicle-verified", authorization: "standard-obd", productionAuthorized: true, collectedByWideDiagnostic: true, operatingStates: ["key-on", "running"], source: "SAE Mode 01 PID 05 / existing Flex decoder" }),
  signal({ key: "engine.map", label: "Imusarjan absoluuttinen paine / MAP", commands: ["010B"], metricIds: ["map"], expectedResponsePrefix: "410B", decoder: "sae-mode01-map", unit: "kPa", plausibleRange: [0, 255], evidence: "vehicle-verified", authorization: "standard-obd", productionAuthorized: true, collectedByWideDiagnostic: true, operatingStates: ["key-on", "running"], source: "SAE Mode 01 PID 0B / existing Flex decoder" }),
  signal({ key: "engine.maf", label: "Ilmamassa / MAF", commands: ["0110"], metricIds: ["maf"], expectedResponsePrefix: "4110", decoder: "sae-mode01-maf", unit: "g/s", plausibleRange: [0, 655.35], evidence: "vehicle-verified", authorization: "standard-obd", productionAuthorized: true, collectedByWideDiagnostic: true, operatingStates: ["running"], source: "SAE Mode 01 PID 10 / existing Flex decoder" }),
  signal({ key: "engine.rail_pressure_obd", label: "Common rail -paine, standardi OBD", commands: ["0123"], metricIds: ["railPressure"], expectedResponsePrefix: "4123", decoder: "sae-mode01-diesel-rail-pressure", unit: "kPa", plausibleRange: [0, 655350], evidence: "vehicle-verified", authorization: "standard-obd", productionAuthorized: true, collectedByWideDiagnostic: true, operatingStates: ["cranking", "running"], source: "SAE Mode 01 PID 23 / existing Flex decoder" }),
  signal({ key: "engine.barometric_pressure", label: "Ilmanpaine / BARO", commands: ["0133"], metricIds: ["barometricPressure"], expectedResponsePrefix: "4133", decoder: "sae-mode01-barometric-pressure", unit: "kPa", plausibleRange: [0, 255], evidence: "vehicle-verified", authorization: "standard-obd", productionAuthorized: true, collectedByWideDiagnostic: true, operatingStates: ["key-on", "running"], source: "SAE Mode 01 PID 33 / existing Flex decoder" }),
  signal({ key: "engine.ecu_voltage", label: "ECU-ohjausmoduulin jännite", commands: ["0142"], metricIds: ["voltage"], expectedResponsePrefix: "4142", decoder: "sae-mode01-control-module-voltage", unit: "V", plausibleRange: [0, 20], evidence: "vehicle-verified", authorization: "standard-obd", productionAuthorized: true, collectedByWideDiagnostic: true, operatingStates: ["key-on", "cranking", "running"], source: "SAE Mode 01 PID 42 / existing Flex decoder" }),
  signal({ key: "engine.egr_commanded_obd", label: "EGR-pyyntö, standardi OBD", commands: ["012C"], metricIds: ["commandedEgr"], expectedResponsePrefix: "412C", decoder: "sae-mode01-commanded-egr", unit: "%", plausibleRange: [0, 100], evidence: "research-candidate", authorization: "standard-obd", productionAuthorized: true, collectedByWideDiagnostic: false, operatingStates: ["running"], source: "SAE Mode 01 PID 2C; component-context fallback only" }),
  signal({ key: "engine.egr_position_obd", label: "EGR-asento, standardi OBD", commands: ["0169"], metricIds: ["egrPositionActual"], expectedResponsePrefix: "4169", decoder: "sae-mode01-egr-data", unit: "%", plausibleRange: [0, 100], evidence: "research-candidate", authorization: "standard-obd", productionAuthorized: true, collectedByWideDiagnostic: false, operatingStates: ["running"], source: "SAE Mode 01 PID 69; component-context fallback only" }),
  signal({ key: "engine.injection_timing_obd", label: "Ruiskutusajoitus, standardi OBD", commands: ["015D"], metricIds: ["injectionTiming"], expectedResponsePrefix: "415D", decoder: "sae-mode01-fuel-injection-timing", unit: "°CA", plausibleRange: [-210, 301.992], evidence: "research-candidate", authorization: "standard-obd", productionAuthorized: true, collectedByWideDiagnostic: false, operatingStates: ["running"], source: "SAE Mode 01 PID 5D; not part of the current standard wide-diagnostic set" }),

  signal({ key: "engine.egr_position_toyota", label: "EGR-venttiilin asento", commands: ["212C"], metricIds: ["toyotaEgrPosition"], expectedResponsePrefix: "612C", decoder: "toyota-2ad-fhv-212c-v1", unit: "%", plausibleRange: [0, 100], evidence: "vehicle-verified", authorization: "vehicle-profile", productionAuthorized: true, collectedByWideDiagnostic: true, operatingStates: ["running"], source: "IS220d profile / vehicle-verified 212C" }),
  signal({ key: "engine.dpnr_differential_pressure", label: "DPNR-paine-ero", commands: ["217E"], metricIds: ["dpnrDifferentialPressure"], expectedResponsePrefix: "617E", decoder: "toyota-2ad-fhv-217e-v1", unit: "kPa", plausibleRange: [-5, 95], evidence: "vehicle-verified", authorization: "vehicle-profile", productionAuthorized: true, collectedByWideDiagnostic: true, operatingStates: ["key-on", "running"], source: "IS220d profile / vehicle-verified 217E" }),
  signal({ key: "engine.egt_inlet", label: "Pakolämpö ennen DPNR:ää", commands: ["217F"], metricIds: ["dpnrInletTemperature"], expectedResponsePrefix: "617F", decoder: "toyota-2ad-fhv-217f-v1:inlet", unit: "°C", plausibleRange: [-40, 1200], evidence: "vehicle-verified", authorization: "vehicle-profile", productionAuthorized: true, collectedByWideDiagnostic: true, operatingStates: ["key-on", "running"], source: "IS220d profile / vehicle-verified 217F" }),
  signal({ key: "engine.egt_outlet", label: "Pakolämpö DPNR:n jälkeen", commands: ["217F"], metricIds: ["dpnrOutletTemperature"], expectedResponsePrefix: "617F", decoder: "toyota-2ad-fhv-217f-v1:outlet", unit: "°C", plausibleRange: [-40, 1200], evidence: "vehicle-verified", authorization: "vehicle-profile", productionAuthorized: true, collectedByWideDiagnostic: true, operatingStates: ["key-on", "running"], source: "IS220d profile / vehicle-verified 217F" }),
...
export function metricIdsForIs220dDiagnosticSignal(key) {
  const definition = getIs220dDiagnosticSignal(key);
  if (!definition) throw new Error(`Unknown IS220d diagnostic signal: ${key}`);
  return definition.metricIds || Object.freeze([]);
}
