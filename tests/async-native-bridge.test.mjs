...
test("APK-transformi siirtää Classic- ja BLE-sendit asynkronisen sillan kautta", () => {
  const patched = patchCoreForAsyncNativeBridge(coreSource);
  assert.match(patched, /async function nativeAsyncBridgeSend\(/);
  assert.match(patched, /await nativeAsyncBridgeSend\(this\.bridge, "classic", String\(command\), Number\(timeoutMs\)\)/);
  assert.match(patched, /await nativeAsyncBridgeSend\(this\.bridge, "ble", String\(hex\), Number\(timeoutMs\)\)/);
  assert.match(patched, /await new Promise\(resolve => setTimeout\(resolve, 25\)\)/);
  assert.match(patched, /raw !== "__PENDING__"/);
});

test("asynkroninen natiivisilta ajaa varsinaisen OBD-sendin taustasäikeessä", () => {
