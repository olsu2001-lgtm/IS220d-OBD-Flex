...
test("0.9.1 versionumero täsmää Android-asetuksissa, paketissa ja käyttöliittymässä", async () => {
  const [app, pkg, main] = await Promise.all([read("app.js"), read("package.json"), read("src/main.js")]);
  assert.match(app, /version:\s*"0\.9\.1"/);
  assert.equal(JSON.parse(pkg).version, "0.9.1");
  assert.match(main, /const APP_VERSION = "0\.9\.1";/);
});
