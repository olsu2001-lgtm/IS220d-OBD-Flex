...
test("0.6.9 ei pyydä internetoikeutta ja säilyttää Flex-pakettitunnuksen", async () => {
  const app = await read("app.js");

  assert.match(app, /packageId:\s*"fi\.oliver\.is220dobd"/);
  assert.match(app, /name:\s*"IS220d OBD Flex"/);
  assert.match(app, /version:\s*"0\.6\.9"/);
  assert.equal(app.includes("android.permission.INTERNET"), false);
});

test("0.6.9:n vLinker BLE -polku löytää FFF1 UARTin, neuvottelee MTU:n ja raportoi GATT-capabilityt", async () => {
  ...
  assert.match(core, /TOYOTA_READ_DATA_ALLOWED_COMMANDS\.includes\(command\)/);
...
test("0.6.9 käyttää lähdekohtaista adaptiivista pollausjonoa ja säilyttää Quicklynks-polun erillisenä", async () => {
...
test("0.6.9 lisää vain lukevan DPNR-tarkistusnäkymän ilman varmentamattomia tilalippuja", async () => {
  const [main, html] = await Promise.all([read("src/main.js"), read("index.html")]);

  assert.match(html, /id="page-dpnr"/);
  assert.match(html, /id="dpnrRaw217e"/);
  assert.match(html, /id="dpnrRaw217f"/);
  assert.match(html, /Thermal Deteriorate/);
  assert.match(main, /DPNR_MONITOR_METRIC_IDS/);
  assert.match(main, /rawLatest:\s*\{\s*\.\.\.state\.rawValues\s*\}/);
});
...
test("automaattinen ELM/CAN-diagnostiikka jatkaa NO DATA -tilanteesta raporttiin asti", async () => {
  ...
  assert.match(main, /TOYOTA_READ_DATA_PROBES/);
  ...
  assert.match(main, /"ATCRA7E8"/);
  assert.match(main, /TOYOTA_READ_DATA_ALLOWED_COMMANDS\.includes\(command\)/);
...
test("Quicklynksin natiivisilta raportoi BLE-ilmoituspalat ja diagnostiikkajako käyttää omaa otsikkoa", async () => {
  ...
  assert.match(main, /IS220d Flex · laaja Quicklynks BLE -diagnostiikka/);
  assert.match(main, /IS220d_Flex_laaja_diagnostiikka_\$\{adapter\}/);
