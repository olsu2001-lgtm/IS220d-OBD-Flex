...
test("Field Test Mode only drives the existing diagnostic UI and has no transport/send implementation", () => {
  ...
  assert.equal((source.match(/diagnosticButton\.click\(\)/g) || []).length, 1);
...
test("Field Test Mode has no network, file export or automatic multi-run loop", () => {
  assert.doesNotMatch(source, /fetch\s*\(|XMLHttpRequest|WebSocket|navigator\.share|showSaveFilePicker|setInterval|setTimeout/);
